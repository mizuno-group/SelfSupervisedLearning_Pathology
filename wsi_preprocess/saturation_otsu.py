...
def get_patch_mask(image_file, patch_size, threshold=None):
    """
    WSIから, 組織の含まれるpatchを取り出すマスクを生成する。
    
    Parameters
    ----------
    image_file: str
        File path of tif image to be processed.
    patch_size: int
        Size of each patch.
    threshold: int or float, optional
        Threshold of saturation to distinguish object and background.
        If None, threshold will be calculated with OTSU method in the image.
    Returns
    -------
    patch_mask: np.array(bool)[shape of wsi]
        Trueになっているセルが組織, Falseになっているセルが背景
    """
    ...
    whole = wsi.read_region(location=(0,0), level=level,
        size = wsi.level_dimensions[level]).convert('HSV')
...
def get_slice_idx(image_file, patch_size, threshold=None, slice_min_patch = 500, 
    ...
    n_slice: int
        切片の総数
    """
    mask = get_patch_mask(image_file, patch_size, threshold)
