...
from src.models import resnet, swav, linearhead
...
parser.add_argument('--seed', type=str, default=24771)
...
def prepare_data():
    ...
    """
    train_data_ssl_transform, train_data_transform, other_data_transform = utils.myrotation(split=True, multi=True)
...
# prepare model
def prepare_model(model, patience:int=7, delta=0, num_classes:int=9):
    """
...
def prepare_model_ssl(modelname:str='TestNet', patience:int=7, delta=0):
    ...
    model = swav.SwaV(
        backbone, head_size=[512*EXPANSION, 512, 128], n_prototypes=512
        )
    model.to(DEVICE)
...
def train_epoch_ssl(model, train_loader, criterion, optimizer):
    ...
    train_batch_loss = []
    for lst_data, _ in train_loader:
        model.prototypes.normalize()
        multi_crops_features = [model(x.to(DEVICE)) for x in lst_data]
...
def main():
    if args.train:
        ...
        utils.summarize_model(
            model, summary(model, next(iter(train_ssl_loader))[0][0].size()), DIR_NAME, lst_name=['summary_ssl.txt', 'model_ssl.pt']
            )
        # 2. learning
