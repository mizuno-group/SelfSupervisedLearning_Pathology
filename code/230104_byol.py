...
from src.models import resnet, byol
...
def prepare_model(model, patience:int=7, delta:float=0., num_classes:int=9):
    ...
    # model building
    encoder=model.online_encoder.net
    encoder.fc=nn.Linear(512*EXPANSION, num_classes) # add head
    encoder = utils.fix_params(encoder, all=False)
    encoder.to(DEVICE)
    ...
    return encoder, criterion, optimizer, scheduler, early_stopping
