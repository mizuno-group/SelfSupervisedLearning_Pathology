...
from src.models import resnet, simsiam, linearhead
...
def prepare_model(model, patience:int=7, delta:float=0., num_classes:int=9):
    ...
    # model building
    model_all = linearhead.LinearConversion(model.encoder, num_classes=num_classes, dim=512*EXPANSION) 
...
def prepare_model_ssl(modelname:str='TestNet', patience:int=7, delta=0.):
    ...
    model = simsiam.SimSiam(encoder, dim=512*EXPANSION)
    model.to(DEVICE)
...
def train_ssl(model, train_loader, criterion, optimizer, scheduler, early_stopping, num_epoch:int=100):
    ...
    for epoch in trange(num_epoch):
        ...
        if early_stopping.early_stop:
            ...
            break
    #early_stopping.delete_checkpoint() # delete checkpoint
